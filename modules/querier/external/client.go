...
type authenticator func(ctx context.Context, endpoint string, req *http.Request) error

func defaultAuthenticator(ctx context.Context, endpoint string, req *http.Request) error {
	// no-op
	return nil
}
...
func NewClient(cfg Config) (*client, error) {
	...
	case "":
		...
		return internalNew(cfg)
	case "cloud-run":
		return internalNew(cfg, WithAuthenticator(cloudRunAuthenticator))
	case "lambda":
		// TODO: implement lambda auth. Some resources:
		...
		return nil, fmt.Errorf("lambda external backend is not yet supported for backend search. Unset the external_backend field to use unauthenticated HTTP instead.")
	default:
		return nil, fmt.Errorf("unknown external backend configured: %s", cfg.Backend)
	}
}

func internalNew(cfg Config, opts ...option) (*client, error) {
...
func (s *client) Search(ctx context.Context, maxBytes int, searchReq *tempopb.SearchBlockRequest) (*tempopb.SearchResponse, error) {
	...
	err = s.authenticator(ctx, endpoint, req)
	if err != nil {
